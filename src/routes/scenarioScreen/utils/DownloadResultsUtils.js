...
export const hideCopayOrCoinsFields = (arr) => {
  let a = arr
    ...
    ?.map((obj1) => ({
      ...
      value: parseFloat(removeSymbolsFromInput(obj1.newValue)),
    }))
...
export const changeDropValuesFromNumericToText = (arr) => {
  let a = [...arr]
  let b = a?.map((item) => {
    if (item.Benefit_Data_Type === 'Drop') {
      if (item.Range?.includes('|')) {
        return {
          ...item,
          currentValue: handlesDropValuesByBenefitGroup(item.currentValue, item.benefitGroup, item.Range),
          newValue: handlesDropValuesByBenefitGroup(item.newValue, item.benefitGroup, item.Range),
        }
      } else {
        return item
      }
...
export const filterScenarioDetailsDataForDownload = (arr) => {
  ...
  let changedResultFromDropNumericToText = changeDropValuesFromNumericToText(result)
  let removeNotRequiredKeys = changedResultFromDropNumericToText?.map(({ Benefit_Data_Type, Range, ...required }) => required)
  return removeNotRequiredKeys
}
