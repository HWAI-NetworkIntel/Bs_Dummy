...
const CustomToggle = ({ record, children, index }) => {
  ...
  const [isToggleOn, setIsToggleOn] = useState(
    benefits[selectedBidId][record.benefitOrder].currentValue.includes('Y') || benefits[selectedBidId][record.benefitOrder].currentValue === '1'
  )
  useEffect(() => {
  ...
  const isValuePresent = (list) => {
    let a = list?.filter(
      (key) =>
        benefits[selectedBidId][key].currentValue.length > 0 &&
        !benefits[selectedBidId][key].currentValue.includes('NC') &&
        benefits[selectedBidId][key].currentValue !== 'Yes' &&
        benefits[selectedBidId][key].currentValue !== 'No'
    )
    if (list?.length > 0) {
  ...
  return (
    <div className="w-full flex flex-wrap gap-x-4 gap-y-2 px-2">
      ...
      {(benefits[selectedBidId][record.benefitOrder].currentValue === 'Yes' || benefits[selectedBidId][record.benefitOrder].currentValue === '1') &&
        children}
    </div>
  )
}
