...
const IntInput = ({ record, hideTitle, titleUp }) => {
  ...
  const saveChangedData = (str) => {
    if (str) {
      ...
      dispatch(
        handleBenefits({ bidId: selectedBidId, index: record.benefitOrder, value: benefits[selectedBidId][record.benefitOrder].currentValue })
      )
    }
  }

  const handleError = useCallback(() => {
    if (benefits[selectedBidId][record.benefitOrder].currentValue) {
      let errorIndexes = []
      data?.forEach((item, i) => {
        if (i < benefits[selectedBidId][record.benefitOrder].currentValue) {
  ...
  // removes greater packages than selected int
  ...
  }, [benefits[selectedBidId][record.benefitOrder].currentValue])
  ...
  // renders packages based on input value
  ...
  }, [benefits[selectedBidId][record.benefitOrder].currentValue, value1])
