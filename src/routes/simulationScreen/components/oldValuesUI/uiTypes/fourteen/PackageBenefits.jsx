...
import { individualBenefitMapper, mapSubBenefitsAsPerUiTypeForUIFourteen } from '../../../../utils/Benefits'

const PackageBenefits = ({ arr, packageNo }) => {
  const { benefitsToShow } = useSelector(({ sectionD }) => sectionD)

  let a = [...arr]?.filter((singleItem) => singleItem.benefitDataType === 'multi select')[0]?.child
  let b = []
  a?.forEach((aa) => {
    ...
    <>
      {benefitsToShow[packageNo]?.map((key) => (
        <div className="flex flex-col w-full">
          ...
          <div className="w-full">{individualBenefitMapper(c[key]?.subBenefits, c[key]?.uiType)}</div>
        </div>
      ))}
    </>
  )
