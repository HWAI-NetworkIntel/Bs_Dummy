...
const CopayCoinsRow = ({ list, title, isOptional, i }) => {
  ...
  let test = list
    ...
    ?.map((x) => benefits[selectedBidId][x.benefitOrder].currentValue)
    .filter((k) => k && k !== 'NC')
  const [isChecked, setIsChecked] = useState(
    benefits[selectedBidId][list[0].dependentOnAdditionalEhc]?.currentValue.split('')[list[0].additionalEhcOrder - 1] === '1'
  )
      ...
      if (isChecked) {
        if (getPreviousValueIfAny) {
          let indexOfChangedItem = list?.findIndex(
            (rowChild) =>
              !benefits[selectedBidId][rowChild.benefitOrder].currentValue.includes('NC') &&
              !(benefits[selectedBidId][rowChild.benefitOrder].currentValue === '')
          )
  ...
  useEffect(() => {
    if (benefits[selectedBidId][list[0].dependentOnAdditionalEhc]?.currentValue.split('')[list[0].additionalEhcOrder - 1] === '1') {
      if (getPreviousValueIfAny) {
        setIsChecked(true)
      }
    } else {
  ...
  }, [getPreviousValueIfAny, benefits[selectedBidId][list[0].dependentOnAdditionalEhc]?.currentValue.split('')[list[0].additionalEhcOrder - 1]])
      ...
      {isOptional ? (
        isChecked && (
          <CopayCoinsCard
            ...
            list={list}
            getPreviousValueIfAny={(benefits[selectedBidId][list[0].dependentOnAdditionalEhc]?.currentValue.split('')[list[0].additionalEhcOrder - 1] === '1') ? getPreviousValueIfAny : null}
          />
        )
      ) : (
