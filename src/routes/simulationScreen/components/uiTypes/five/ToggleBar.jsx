...
const ToggleBar = ({ item }) => {
      ...
      if (!isChecked) {
        return (
          <div className="flex flex-col w-full gap-4">
            {item.child?.map((el, i) => (
              <DataTypeMapper item={el} key={i} i={i} />
            ))}
          </div>
        )
      }
  ...
  const handlerEhcValues = (listToHandle) => {
    listToHandle?.forEach((element) => {
      ...
      if (isChecked === false) {
        ...
        copayCoinsList.forEach((obj, i) => {
          dispatch(
            handleBenefits({ bidId: selectedBidId, index: obj.benefitOrder, value: benefits[selectedBidId][obj.benefitOrder]?.newValue })
          )
          if (i === 0 || i === 2) {
            if (obj?.dependentOnEhc !== 0) {
              dispatch(
  ...
  const handleErrorCaseForEhc = useCallback(
    (itemNo) => {
      if (isChecked) {
        let listOfBinaryCodes = [...benefits[selectedBidId][itemNo]?.newValue.split('')]
  ...
  const handleErrorCaseForUnlimited = useCallback(() => {
    if (!isChecked) {
      let index = item.child?.findIndex((el) => benefits[selectedBidId][el.benefitOrder]?.newValue === '')
      if (index !== -1) {
        dispatch(addToErrorBenefitNamesList(`${item.benefitGroup}-${item.benefitOrder}`))
      } else {
        dispatch(removeFromErrorBenefitNamesList(`${item.benefitGroup}-${item.benefitOrder}`))
  ...
  const handleErrorCaseForNonOptional = useCallback(() => {
    if (isChecked) {
      let i = listOfInputs?.findIndex(
        (element) => benefits[selectedBidId][element.benefitOrder].currentValue !== benefits[selectedBidId][element.benefitOrder].newValue
      )
      if (i !== -1) {
        dispatch(removeFromErrorBenefitNamesList(`${item.benefitGroup}-${item.benefitOrder}`))
      } else {
        let ii = listOfInputs?.findIndex(
          (element) =>
            benefits[selectedBidId][element.benefitOrder].newValue !== '' &&
            !benefits[selectedBidId][element.benefitOrder].newValue.includes('NC')
        )
        if (ii !== -1) {
          dispatch(removeFromErrorBenefitNamesList(`${item.benefitGroup}-${item.benefitOrder}`))
        } else {
  ...
  return (
    <div className="w-full flex flex-wrap gap-x-4 gap-y-2">
      <div className="flex w-full overflow-hidden shadow-cards p-2 justify-between items-center gap-x-10 ">
        <p
          style={{
            color: `${benefits[selectedBidId][item.benefitOrder].currentValue !== benefits[selectedBidId][item.benefitOrder].newValue &&
