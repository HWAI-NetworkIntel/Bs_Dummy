...
const CustomInput = ({ record, hideTitle, titleUp }) => {
  ...
  useEffect(() => {
    if (benefits[selectedBidId][record.benefitOrder].currentValue !== benefits[selectedBidId][record.benefitOrder].currentValue) {
      setValue(benefits[selectedBidId][record.benefitOrder].currentValue?.replace('$', '')?.replace(',', '')?.replace('%', ''))
    }
  }, [])
  const title = (
  ...
  const saveChangedData = () => {
    dispatch(
      handleBenefits({
        ...
        value: changedBenefit.currentValue,
      })
    )
  }
      ...
      switch (record.benefitDataType) {
        ...
        case BENEFIT_DATA_TYPES.ncPercent:
          ...
          if (record.currentValue?.split('%')[0] !== finalValue) {
            ...
            // setChangedBenefit({
            //   ...changedBenefit,
            //   currentValue: finalValue !== 'NC' ? finalValue + '%' : finalValue,
            // })
          }
          if (record.currentValue?.split('%')[0] === finalValue) {
            dispatch(
        ...
        case BENEFIT_DATA_TYPES.ncMoney:
          ...
          if (record.currentValue?.split('$')?.join(EMPTY_STRING)?.split(',')?.join(EMPTY_STRING) !== finalNcMoney) {
            ...
            // setChangedBenefit({
            //   ...changedBenefit,
            //   currentValue: finalNcMoney !== 'NC' ? '$' + finalNcMoney : finalNcMoney,
            // })
          }
          if (record.currentValue?.split('$')?.join(EMPTY_STRING)?.split(',')?.join(EMPTY_STRING) === finalNcMoney) {
            dispatch(
  ...
  return (
    <div>
      <div
        className={`flex w-full ${hideTitle ? 'justify-center' : 'justify-between'} p-2 ${
          titleUp ? 'items-start' : 'items-center'
      ...
      {record.benefitDataType === 'Int' &&
        Number(value1) > 0 &&
        record.child?.map((innerChild, i) => <DataTypeMapper item={innerChild} key={i} i={i} />)}
    </div>
  )
}
