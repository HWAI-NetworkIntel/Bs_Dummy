...
const OneIntInput = ({ item, hideTitle, titleUp }) => {
  ...
  const [value1, setValue] = useState(benefits[selectedBidId][item.benefitOrder].currentValue)
  ...
  const handleError = useCallback(() => {
    if (benefits[selectedBidId][item.benefitOrder].currentValue) {
      let errorIndexes = []
      data?.forEach((itemObj, i) => {
        if (i < parseFloat(benefits[selectedBidId][item.benefitOrder].currentValue) - 1) {
          let emptyValueIndex = itemObj.items?.findIndex((obj) => benefits[selectedBidId][obj.benefitOrder].currentValue === '')
  ...
  const childContent = useCallback(() => {
    let no = benefits[selectedBidId][item.benefitOrder].currentValue
    if (no) {
          ...
          >
            {item.range === '1|4' ? (
              ...
              <Input
                ...
                placeholder={benefits[selectedBidId][item.benefitOrder].currentValue}
