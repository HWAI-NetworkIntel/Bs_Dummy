...
import { benefitsToShow } from '../../../../utils/Fifteen'
import { setBenefitsToShow } from '../../../../reducer/UiFifteen'

const MultiSelectNine = ({ item, packageNo }) => {
  const dispatch = useDispatch()
  const { selectedBidId, errorBenefitNameList, benefits } = useSelector(({ benefits }) => benefits)
  const newValues = item.currentValue.split(';').filter((str) => str)
  ...
  useEffect(() => {
    if (benefits[selectedBidId]?.[item.benefitOrder]?.currentValue) {
      let arrOfValues = benefits[selectedBidId]?.[item.benefitOrder]?.currentValue.split(';').filter((str) => str)
